...
# 虽然名字是csv，但实际上是制表符分隔
otu_raw <- read.csv("bacteria_data_rarefied_9480.csv", header = TRUE,row.names = 1,stringsAsFactors = FALSE,check.names = FALSE)
# 2. 读取分组信息
...
p <- ggplot(pcoa_data, aes(x = PCoA1, y = PCoA2)) +
  ...
  labs(x = paste0("PCoA1 (", eig_percent[1], "%)"),
       ...
       title = "Structure of Endolithic Bacterial Communities",
